...
const data = [
    {
        ...
        title: 'Free',
        price: '0',
        description: "please try with free trial"
        ...
        <>
            <div className='flex flex-col items-center w-full mx-auto min-h-screen digonal-background overflow-x-hidden'>
                <div className='flex justify-between items-center w-full px-6 h-20 bg-[#00000012'>
                    <div className='text-4xl font-bold text-white'> {planType} </div>
                    <div className='flex justify-center items-center gap-2'>
                        {
                            !userId ? (
                                <a href='/login' className='bg-white px-4 py-2 uppercase w-auto rounded-lg text xl text-[#4f7cff] font-semibold'>
                                    Login
                                </a>
                            ) : (
                                <div className='flex justify-center items-center space-x-4'>
                                    <span className='text-white text-xl'>{planType}</span>
                ...
                <div className='grid lg: grid-cols-2 sm: grid-cols-2 grid-cols-1 gap-[10%] z-50 place-items-center w-9/12 max-w-[1000px] mx-[500px] mt-20'>
                    {data.map((item, idx) => (
                        <div key={idx} className='bg-white px-6 py-8 rounded-xl h-[500px] text-[#4f7cff] w-full mx-auto grid place-items-center'>
                            <div className='font-bold text-[40px]'> {item.title}</div>
                            <img src={item.src} alt='' width={200} height={300} className='h-300'>
                            </img>
